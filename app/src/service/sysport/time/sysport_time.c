...
static const char strTimeLogName[] = "time";

#define sysport_time_log( strFormat, ...) \
    do{ \
        storage_log( strTimeLogName, LOG_LEVEL_NOTICE, strFormat, ##__VA_ARGS__); \
    }while(0)

#define sysport_time_debug( strFormat, ...) \
    do{ \
        storage_log( strTimeLogName, LOG_LEVEL_DEBUG, strFormat, ##__VA_ARGS__); \
    }while(0)
