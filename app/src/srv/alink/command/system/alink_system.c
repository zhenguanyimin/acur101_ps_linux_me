...
extern char* eth_protocol_get_sn( void );
static void system_get_info( alink_payload_t *psPayload, alink_resp_t *psResp )
{
	...
	switch( psGetInfo->type )
	{
		...
		case 1:
			...
			strcpy( (char*)&psGetInfoAck->value[1], eth_protocol_get_sn() );
			psResp->wCount = sizeof(alink_sys_get_info_ack_t);
			break;
		case 2:
			psGetInfoAck->type = 2;
...
static void system_set_sn( alink_payload_t *psPayload, alink_resp_t *psResp )
{
	...
	int eRet = FlashRaw_Set( "sn", psSetSn->sn, strlen(psSetSn->sn) );
...
static void system_get_amp_code( alink_payload_t *psPayload, alink_resp_t *psResp )
{
	...
	get_amp_code_store( (uint16_t *)psResp->pbyBuffer, &wLen );
	printf( "[%s %d]\r\n", __func__, __LINE__ );
...
static void system_get_amp_data( alink_payload_t *psPayload, alink_resp_t *psResp )
{
	...
	get_amp_data_store( (uint16_t *)psResp->pbyBuffer, &wLen );
	printf( "[%s %d]\r\n", __func__, __LINE__ );
	for( int i = 0; wLen > i; ++i )
		printf( " %02X", psResp->pbyBuffer[i] );
	printf( "\r\n" );
...
static void system_get_phase_data( alink_payload_t *psPayload, alink_resp_t *psResp )
{
	...
	/*
	* 2023.8.22, tangsongquan: wLen's type is not valid passed to function, modify to same type of data.
	*/
	phase_data_store( (uint16_t *)psResp->pbyBuffer, wLen );
	printf( "[%s %d]\r\n", __func__, __LINE__ );
	for( int i = 0; wLen > i; ++i )
		printf( " %02X", psResp->pbyBuffer[i] );
	printf( "\r\n" );
...
static uint16_t system_pkg_heartbeat( uint8_t *pbyBuffer, uint16_t wSize, void *psStatus )
{
	...
	psHeartBeat->electricity	= 80;
	psHeartBeat->status			= eSysStatus;	//need read
...
static alink_cmd_list_t sAlinkCmd_SystemGetAmpCode = { 0 };
static alink_cmd_list_t sAlinkCmd_SystemGetAmpData = { 0 };
static alink_cmd_list_t sAlinkCmd_SystemGetPhaseData = { 0 };
...
void alink_system_init( void )
{
	...
	alink_register_cmd( &sAlinkCmd_SystemGetAmpCode,	0xD6,	0,									system_get_amp_code );
	alink_register_cmd( &sAlinkCmd_SystemGetAmpData,	0xD7,	0,									system_get_amp_data );
	alink_register_cmd( &sAlinkCmd_SystemGetPhaseData,	0xD8,	0,									system_get_phase_data );
